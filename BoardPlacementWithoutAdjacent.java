...
public class BoardPlacementWithoutAdjacent {
        ...
        for(int i = 0; i < board.getBoard().length; i++) {
            for( int j = 0; j < board.getBoard().length; j++) {
                if(word.contains(Character.toString(board.getTile(i,j)))){
                    if(board.getTileLeft(i,j) == ' ' && board.getTileRight(i,j) == ' ' && board.adjacentClearHorizontal(i, j - word.indexOf(board.getTile(i,j)), j, word.length()) == true) {
                        for(int k =0; k < word.length(); k++) {
                            board.setTile(i, j - word.indexOf(board.getTile(i,j)) + k, word.charAt(k));
                        }
                        return;
                    }
                    if(board.getTileUp(i,j) == ' ' && board.getTileDown(i,j) == ' ' && board.adjacentClearVertical(j, i - word.indexOf(board.getTile(i,j)), i, word.length()) == true) {
                        for(int k =0; k < word.length(); k++) {
